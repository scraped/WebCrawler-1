...
class Parser
	...
	getLinks: (body, domain) ->
		new Promise (resolve) =>
			urls = []
			@$ = Cheerio.load @removeScriptsTags body
	...
	getDomain: (url) ->
		parts = urlParser(url).hostname.toString().split '.'
		if parts
	...
	getSitemap: (domain) =>
		new Promise (resolve, reject) =>
			sitemap.fetch("#{domain}/sitemap.xml")
			.then (sitemap) => resolve sitemap
	...
	getTechnologies: (url) ->
		new Promise (resolve, reject) =>
			wappalyzer.detectFromUrl url: url, (err, apps, appInfo) =>
				resolve apps, appInfo if !err
				reject err if err

	getWords: (body, langs, limit = null) ->
		words = []
		corpus = []
		text  = @$('body').text()
	...
	url: (url, domain) ->
		if url and url isnt undefined
			...
			if !url.host and !url.hostname and !url.protocol
